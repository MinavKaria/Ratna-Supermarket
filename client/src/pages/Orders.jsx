...
function Orders() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `https://ratna-supermarket.vercel.app/allOrder/${userID}`,
          {
            headers: {
  ...
  const generatePDFInvoice = async (order) => {
    ...
    const watermarkImageArrayBuffer =
      await watermarkImageResponse.arrayBuffer();
      ...
      <div className="mt-[120px] flex justify-center">
        <div className="container w-1/2 shadow-lg p-4">
          ...
          {orders.map((order, index) => (
            <div
              key={index}
              className="mb-4 border border-gray-300 p-4 flex justify-between"
            >
              <div>
                <ul>
