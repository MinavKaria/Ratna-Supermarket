...
function Signup() {
  ...
  const signInUser = async () => {
    ...
    try {
      ...
      setIsLogin(true);
      setName(name); // Changed from user.name to name from response
          ...
          <Grid container spacing={2}>
            <Grid item xs={6}>
              <UserDataContainer onSubmit={async (e) => {
                e.preventDefault();
                if (isSignup) { // Changed from !isSignup to isSignup
                ...
                <Typography variant="h4" gutterBottom>
                  {isSignup ? "Sign Up" : "Sign In"} {/* Correct labels based on isSignup */}
                </Typography>
                <br />
                {isSignup && ( // Show sign-up fields when isSignup is true
                  <>
                    <Input
                      type="text"
                ...
                <button
                  ...
                  type="button"
                >
                  {isSignup ? 'Already have an account?' : 'Create an Account'} {/* Swapped labels */}
                </button>
                <br />
                <button className="bg-green-600 w-32 p-2 rounded-2xl text-white" type='submit'>
                  {isSignup ? 'Sign Up' : 'Sign In'} {/* Correct submit button labels */}
                </button>
                <br/>
                {/* <GoogleLogin
                    onSuccess={()=>{
