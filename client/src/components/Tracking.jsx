    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://ratna-supermarket.vercel.app/order/${orderID}`,
  ...
  return (
    <>
      <div className="container mx-auto mt-44 ">
        <h2 className="px-[11rem] text-2xl mb-14 font-bold">
          Order Tracking for Order ID {orderID}
        </h2>
        <Box sx={{ width: "100%" }}>
        ...
        <div className="container mx-auto hidden md:flex justify-center w-40 gap-52 mb-32 ">
          <img
            src="/delivery_1.svg"
            alt=""
            className={`${activeStep < 1 ? "filter grayscale opacity-50" : ""}`}
          />
          <img
            src="/delivery_2.svg"
            alt=""
            className={`${activeStep < 2 ? "filter grayscale opacity-50" : ""}`}
          />
          <img
            src="/delivery_3.svg"
            alt=""
            className={`${activeStep < 3 ? "filter grayscale opacity-50" : ""}`}
          />
          <img
            src="/delivery_4.svg"
            alt=""
            className={`${activeStep < 4 ? "filter grayscale opacity-50" : ""}`}
          />
        </div>
